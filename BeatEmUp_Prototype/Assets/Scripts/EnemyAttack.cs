...
public class EnemyAttack : MonoBehaviour {
	...
	void Start () {
		attackWaitTime = 0; //When the enemy attacks, it sets the coolDown and he/she must wait until the timer is done
	...
	private void Attack() {
		...
		//The dot product returns a number between 1 & -1, if player is in front of us, +#, if player is behind us, -#, and then if he's on either side, it returns 0
		
		//Debug.Log(direction); //Testing info outputs to console
		
		if (distance < 2.5f) { //Using decimals with floats, put f at the end
			if (direction > 0) {
				PlayerHealth pHealth = (PlayerHealth)target.GetComponent("PlayerHealth"); //Grab reference to PlayerHealth script
				pHealth.AdjustCurrentHealth(-10); //Take away 10 health each time the enemy is hit
			}
		}
	}
}
